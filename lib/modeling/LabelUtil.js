import { getBusinessObject, isAny, is } from '../util/Util';
...
export function getLabelAttribute(element) {
  const businessObject = getBusinessObject(element);
  if (isAny(businessObject, [`${MODELER_PREFIX}:Entity`, `${MODELER_PREFIX}:Relationship`, `${MODELER_PREFIX}:Attribute`])) {
    return 'name';
  } else if (isAny(businessObject, [`${MODELER_PREFIX}:Line`]) && !connectsAttribute(businessObject)) {
    return 'cardinality';
  }
}
...
export function requiresExternalLabel(element) {
  const businessObject = getBusinessObject(element);
  return is(element, `${MODELER_PREFIX}:Line`) && !connectsAttribute(businessObject);
}

// Check whether a line is connected to an attribute.
export function connectsAttribute(businessObject) {
  return is(businessObject.source, `${MODELER_PREFIX}:Attribute`) || is(businessObject.target, `${MODELER_PREFIX}:Attribute`);
}

export function mayHaveLabel(element) {
  return !(is(element, `${MODELER_PREFIX}:Line`) && connectsAttribute(element.businessObject));
}
