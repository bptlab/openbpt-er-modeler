...
import { existsExternalLabel, getExternalLabelMid, getLabel, requiresExternalLabel, mayHaveLabel } from "./LabelUtil";
...
export default class CustomLabelEditing {
  constructor(eventBus, canvas, directEditing, commandStack, textRenderer) {
    ...
    eventBus.on('element.dblclick', function (event) {
      // Do not activate direct editing for lines that connect attributes
      if (mayHaveLabel(event.element)) {
  ...
  activate(element) {
    ...
    if (isAny(element, [
      `${MODELER_PREFIX}:Entity`,
      `${MODELER_PREFIX}:Relationship`,
    ...
    if (isAny(element, [`${MODELER_PREFIX}:Line`])) {
      assign(options, {
        autoResize: true
      });
    }
