...
export default class CustomRuleProvider extends RuleProvider {
  ...
  init() {
    ...
    this.addRule('connection.start', function (context) {
      ...
      return is(source, `${MODELER_PREFIX}:Node`) && { type: `${MODELER_PREFIX}:Line` };
    });
    ...
    this.addRule('shape.resize', function (context) {
      if (isAny(context.shape, [`${MODELER_PREFIX}:Entity`, `${MODELER_PREFIX}:Relationship`, `${MODELER_PREFIX}:Attribute`])) {
        return true;
      }
      return false;
    });
...
function canConnect(source, target) {
  return (
    isAny(source, [`${MODELER_PREFIX}:Entity`, `${MODELER_PREFIX}:Relationship`, `${MODELER_PREFIX}:Attribute`]) &&
    isAny(target, [`${MODELER_PREFIX}:Entity`, `${MODELER_PREFIX}:Relationship`, `${MODELER_PREFIX}:Attribute`]) &&
    source.type !== target.type
  ) && { type: `${MODELER_PREFIX}:Line` };
}
